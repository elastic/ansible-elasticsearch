...
`7.0.1-multi` is the last release to support installation of multiple elasticsearch instances in the same hosts.
...
Future evolution will be based on the future `7.0.1` release without multi-instance support (coming soon).

It is still possible to use `7.0.1-multi` version of ansible-elasticsearch role with other Elasticsearch possible by overriding `es_version` variable (for example: `es_version: 6.7.2`).
...
```sh
ansible-galaxy install elastic.elasticsearch,7.0.1-multi
```
